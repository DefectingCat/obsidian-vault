...
1. `FnOnce` ，该类型的闭包捕获变量后获取了其所有权（例如返回了该变量、使用了 `move` 等）；
...
一个闭包函数同时可以实现 3 个特征中的多个，`sum` 函数通过 `move` 关键字来获取了 `str` 的所有权（实现了 `FnOnce`），同时在函数体内它并没有对 `str` 进行修改（实现了 `Fn`）。所以该函数同时实现了多个特征。
